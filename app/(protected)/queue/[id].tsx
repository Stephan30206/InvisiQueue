...
export default function QueueScreen() {
  ...
  const handleJoin = async () => {
    ...
    if (!position) {
      Alert.alert(t("geo.location_required"), t("geo.permission_denied"));
      return;
    }
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      {notification && (
        ...
        >
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ color: colors.background, fontWeight: "700", fontSize: 13 }}>{t("notifications.notification")}</Text>
        ...
        {myEntry ? (
          <View style={{ margin: 20, padding: 24, borderRadius: 16, backgroundColor: colors.text, alignItems: "center" }}>
            ...
            <Text style={{ color: "rgba(255,255,255,0.7)", fontSize: 14, marginBottom: 16 }}>
              {peopleAhead === 0 ? t("queue.served") : `${peopleAhead} ${peopleAhead! > 1 ? t("queue.people_ahead") : t("queue.people_ahead")} ${t("queue.please_come_forward")}`}
            </Text>
          ...
          <View style={{ margin: 20, padding: 24, borderRadius: 16, borderWidth: 1, borderColor: colors.border, alignItems: "center", backgroundColor: colors.surfaceLight }}>
            ...
            <Text style={{ fontSize: 16, fontWeight: "700", color: colors.text, marginBottom: 4 }}>
              {entries.length} {entries.length === 1 ? t("queue.waiting") : t("queue.waiting")}
            </Text>
            <Text style={{ fontSize: 13, color: colors.textMuted, marginBottom: 16 }}>
              ~{entries.length * 3} {t("queue.estimated_time")}
            </Text>
            <TouchableOpacity
              onPress={handleJoin}
              disabled={joining}
          ...
          >
            ...
            {myEntry?.id === entry.id && (
              <View style={{ backgroundColor: colors.text, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6 }}>
                <Text style={{ color: colors.background, fontSize: 10, fontWeight: "700" }}>YOU</Text>
              </View>
            )}
          </View>
        ))}
      ...
      <Modal visible={showGuestModal} transparent animationType="slide" onRequestClose={() => setShowGuestModal(false)}>
        <View style={{ flex: 1, justifyContent: "flex-end", backgroundColor: "rgba(0,0,0,0.5)" }}>
          <View style={{ backgroundColor: colors.surfaceLight, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, gap: 12 }}>
            ...
            <Text style={{ fontSize: 20, fontWeight: "800", color: colors.text }}>{t("queue.join_as_guest")}</Text>
            <Text style={{ fontSize: 13, color: colors.textMuted }}>{t("queue.enter_details")}</Text>
            <TextInput
